...
int main() {
	...
	//auto b_int = std::back_inserter (collection);

	//Алгоритмы копии (берем из четные числа из вектора)
	std::vector<int> evens; //подготовили место для вставки

	std::copy_if(collection2.begin(), collection2.end(), //сообщаем с каким методом коллекции работаем
		std::back_inserter(evens), //функтор связанный с коллекций, кот при обращее к себе размещает новое значение в конце коллекции
		[](const int& a)->bool {return a % 2 == 0; }); //загон в лямбда выражение
	for (auto& el : evens) // Вывод копии
	{
		std::cout << el << ' ';
	}
	std::cout << std::endl;
